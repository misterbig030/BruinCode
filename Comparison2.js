	...
	database = firebase.database();
  	...
  	ref.on("value", function(snapshot){
		...
		btn1.onclick = function(){
		  ...
		  //   var scheduleKeys = Object.keys(scheduleData);
		  //   //alert(data[keys[0]].professor);
		  //   const temp = scheduleKeys.length;
		  //   //console.log(keys.length);
		  //   for (var i = 0; i < temp-1; i++){
		  //     if ((scheduleData[scheduleKeys[i]].professor == data[keys[key1]].professor) && (scheduleData[scheduleKeys[i]].courseNumber == data[keys[key1]].courseNumber)){
		  //       added = true;
		  //       alert("Class already added to schedule");
		  //       break;
		  //     }
		  //     //console.log(added);
		  //   }
